...
export const generateHistoricalData = () => {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        data[dateKey] = generateDayData(dateKey);
    }
    return data;
};

// Safe Getter with Caching to prevent infinite loops
export const getMockData = (dateKey) => {
    if (!dateKey) return historicalData[todayKey]; // Guard against empty
    if (historicalData[dateKey]) return historicalData[dateKey];
    
    // Generate, Cache, and Return
    const newData = generateDayData(dateKey);
    historicalData[dateKey] = newData; // Cache it!
    return newData;
};
