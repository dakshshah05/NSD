...
import { Menu, Bell, Sun, Moon, Search, Calendar, CheckCheck, X } from 'lucide-react';
    ...
    <header className="h-16 bg-[rgb(var(--bg-card))]/80 backdrop-blur-md border-b border-[rgb(var(--border))] px-4 md:px-6 flex items-center justify-between shrink-0 transition-all duration-300 z-50 sticky top-0">
      ...
      <div className="flex items-center space-x-3 md:space-x-6">
        ...
        <div className="hidden md:flex items-center relative">
          <Search size={16} className="absolute left-3 text-[rgb(var(--text-muted))]" />
          <input 
            type="text"
        ...
        <div className="flex items-center space-x-2 md:space-x-4">
          ...
          >
            {isDarkMode ? <Sun size={20} /> : <Moon size={20} className="text-[rgb(var(--text-main))]" />}
          </button>
                ...
                <div className="absolute right-0 top-12 w-80 bg-[rgb(var(--bg-card))] border border-[rgb(var(--border))] rounded-xl shadow-2xl overflow-hidden animate-fade-in origin-top-right">
                    <div className="p-3 border-b border-[rgb(var(--border))] flex justify-between items-center bg-[rgb(var(--bg-input))]/50">
                        <div className="flex items-center gap-2">
                             <h3 className="text-sm font-bold text-[rgb(var(--text-main))]">Notifications</h3>
                             {unreadCount > 0 && <span className="bg-emerald-500 text-white text-[10px] px-1.5 rounded-full">{unreadCount}</span>}
                        </div>
                        <div className="flex items-center gap-2">
                            {notifications.length > 0 && (
                                <button onClick={clearNotifications} className="text-[10px] font-medium text-red-400 hover:text-red-300 transition-colors uppercase tracking-wider">
                                    Clear
                                </button>
                            )}
                            <button onClick={() => setShowNotifs(false)} className="text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text-main))]"><X size={16}/></button>
                        </div>
                    </div>
                    <div className="max-h-64 overflow-y-auto scrollbar-thin scrollbar-thumb-[rgb(var(--border))]">
                        {notifications.length > 0 ? (
                            notifications.slice(0, 5).map(n => (
                                <div key={n.id} className={`p-3 border-b border-[rgb(var(--border))]/50 hover:bg-[rgb(var(--bg-input))] transition-colors ${!n.read ? 'bg-emerald-500/5' : ''}`}>
                                    <div className="flex justify-between items-start">
                                        <h4 className="text-sm font-medium text-[rgb(var(--text-main))]">{n.title}</h4>
                                        <span className="text-[10px] text-[rgb(var(--text-muted))]">{n.time}</span>
                                    </div>
                                    <p className="text-xs text-[rgb(var(--text-muted))] mt-1">{n.message}</p>
                                </div>
                            ))
                        ) : (
                            <div className="p-8 text-center text-[rgb(var(--text-muted))] text-sm">No new notifications</div>
                        )}
                    </div>
