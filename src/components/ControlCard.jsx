...
const Toggle = ({ enabled, onChange, disabled }) => (
  <button
    ...
    className={clsx(
      ...
      enabled ? (disabled ? "bg-emerald-600/50" : "bg-emerald-500") : "bg-slate-700",
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-lg hover:border-slate-700 transition-all">
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400">{room.block}</p>
        </div>
        <div className={clsx(
            "p-2 rounded-lg transition-colors",
            isAutomationOn ? "bg-emerald-500/10 text-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.2)]" : "bg-slate-800 text-slate-500"
        )}>
            <Bot size={20} />
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between p-3 bg-slate-800/50 rounded-lg border border-slate-700">
          <div className="flex items-center space-x-3">
            ...
            <div>
               <p className="text-sm font-medium text-white">Smart Automation</p>
               <p className="text-[10px] text-slate-400">AI optimizes controls</p>
            </div>
          </div>
        ...
        <div className={clsx("flex items-center justify-between transition-opacity", isAutomationOn && "opacity-50")}>
          <div className="flex items-center space-x-3">
             <Lightbulb size={18} className={room.lights ? "text-yellow-400 fill-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.5)]" : "text-slate-600"} />
             <span className="text-sm text-slate-200">Lights</span>
          </div>
          <Toggle 
            enabled={room.lights} 
        ...
        <div className={clsx("flex items-center justify-between transition-opacity", isAutomationOn && "opacity-50")}>
          <div className="flex items-center space-x-3">
             <Fan size={18} className={room.fans ? "text-cyan-400 animate-spin-slow drop-shadow-[0_0_8px_rgba(34,211,238,0.5)]" : "text-slate-600"} />
             <span className="text-sm text-slate-200">Fans</span>
          </div>
          <Toggle 
            enabled={room.fans} 
